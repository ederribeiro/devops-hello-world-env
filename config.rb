...
if ARGV[0].eql?("up")
 ...
 data = YAML.load(IO.readlines("cloud-config.dev")[1..-1].join)
 ...
 yaml = YAML.dump(data)
 File.open("cloud-config", "w") { |file| file.write("#cloud-config\n\n#{yaml}") }
end
#
...
$env_dir = File.dirname(File.realpath(__FILE__))
$shared_folders = { $env_dir => "/var/hello-world/env" }
